<template>
  <div class="display-area">
    <div class="point">
      <span>序号: {{currentNum+1}}</span>
      <span style="margin-left:80px;">当前检定点: {{records[currentNum].point}}μL</span>
    </div>
    <div class="left">
    ...
    <div class="record" style="margin-top:100px;">
      ...
      <el-button
        type="primary"
        plain
        style="width:200px;height:60px;font-size:26px;"
        @click="test"
        :disabled="isWork"
      >开始</el-button>
      <el-button
  ...
  methods: {
    ...
    onExport() {
      let exlBuf = fs.readFileSync(
        `${window.pathName}\\模板\\移液器检定原始记录.xlsx`
      );
      let data = {
      ...
      ejsExcel.renderExcel(exlBuf, data).then(outBuf => {
        console.log(
          ...
          `${window.pathName}\\原始记录\\${
            this.basicInfo[this.sampleIndex].certCode
          }.xlsx`
        );
        fs.writeFileSync(
          `${window.pathName}\\原始记录\\${
